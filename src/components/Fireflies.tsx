...
interface Firefly {
  ...
  breatheDuration: number; breatheDelay: number;
  floatDuration: number; floatDelay: number; floatPath: string;
}
...
export default function Fireflies() {
  ...
  useEffect(() => {
    const generated: Firefly[] = Array.from({ length: 40 }).map((_, i) => ({
      id: i,
      top: `${Math.random() * 100}%`,
  ...
  return (
    <div className="fixed inset-0 w-full h-full pointer-events-none z-10 overflow-hidden mix-blend-screen">
      ...
      `}</style>
      {flies.map(fly => (
        <div key={fly.id} className="absolute" style={{ top: fly.top, left: fly.left, animation: `${fly.floatPath} ${fly.floatDuration}s ease-in-out infinite`, animationDelay: `${fly.floatDelay}s` }}>
          <div className="rounded-full" style={{ width: `${fly.size}px`, height: `${fly.size}px`, backgroundColor: 'rgba(200, 255, 200, 0.9)', animation: `fireflyBreathe ${fly.breatheDuration}s ease-in-out infinite`, animationDelay: `${fly.breatheDelay}s` }} />
        </div>
      ))}
    </div>
  );
